...
def create_or_load_template():
    ...
    template_path = os.path.join(config.FOLDER_PATH, 'survey_template.json')
    ...
    if os.path.exists(template_path):
        ...
        # Create new template
        with open(os.path.join(config.FOLDER_PATH, 'data_collections_basic_basic_petition_letter.json'), 'r') as f:
...
def load_question_mapping():
    ...
    with open(os.path.join(config.FOLDER_PATH, 'survey_questions_mapping_v1.json'), 'r') as f:
        return json.load(f)
